...
test('painel salva filtros, destinos e testa o Radar',async()=>{const [html,script]=await Promise.all([read('../extensao-linkedin/dashboard.html'),read('../extensao-linkedin/dashboard.js')]);assert.match(html,/id="stack-options"/);assert.match(html,/id="download-folder"/);assert.match(html,/id="send-radar"/);assert.match(html,/id="test-radar"/);assert.match(script,/chrome\.storage\.local\.set/);assert.match(script,/selectedStacks/);assert.match(script,/portalToken/);assert.match(script,/action:'test'/);assert.match(script,/Conexão confirmada/)});
...
test('manifest limita integração externa ao Radar conhecido',async()=>{const manifest=JSON.parse(await read('../extensao-linkedin/manifest.json'));assert.equal(manifest.version,'2.1.1');assert.ok(manifest.permissions.includes('storage'));assert.ok(manifest.permissions.includes('downloads'));assert.deepEqual(manifest.host_permissions,['https://www.linkedin.com/*','https://radar-carreira-almir-v2.prof-andreiamr.chatgpt.site/*'])});
